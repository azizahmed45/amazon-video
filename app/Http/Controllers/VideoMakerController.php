...
class VideoMakerController extends Controller
{
    ...
    {
        $image = Image::make(storage_path('app/template/title-template.png'));
        ...
        $image_path = 'app/images/' . Str::uuid() . ".png";
        ...
        $ffmpegCommand = env('FFMPEG_BINARIES') . " -loop 1 -i " . storage_path($image_path) . " -i " . storage_path($audio_path) . " -c:v png -c:a aac -strict experimental -b:a 192k -pix_fmt rgba -shortest -fflags shortest -max_interleave_delta 100M " . storage_path($video_path);
    ...
    {
        $primaryTemplate = Image::make(storage_path('app/template/primary_image.png'));
    ...
    public static function generateImagesFromProduct(Product $product)
    {
        ...
        foreach ($product->scripts as $index => $script) {
            $template = Image::make(storage_path('app/template/product_image.png'));
    ...
    public static function generateVideoForProduct(Product $product)
    {
        ...
        for ($i = 0; $i < count($audios); $i++) {
            ...
            $video_path = 'app/videos/' . Str::uuid() . ".mp4";
            $ffmpegCommand = env('FFMPEG_BINARIES') . " -loop 1 -i " . storage_path($images[$i]->name) . " -i " . storage_path($audios[$i]->name) . " -c:v png -c:a aac -strict experimental -b:a 192k -pix_fmt rgba -shortest -fflags shortest -max_interleave_delta 100M " . storage_path($video_path);
    ...
    public static function mergeBackGroundAudio(Keyword $keyword)
    {
        ...
        $ffmpegCommand = env('FFMPEG_BINARIES') . " -i " . storage_path($video->name) . " -i " . storage_path($background_audio_path) . " -i " . storage_path($bgVideo) . " -filter_complex \"[2:v][0:v]overlay=x=0:y=0[overlay];[0:a]volume=1[a1];[1:a]volume=0.2[a2];[a1][a2]amix=inputs=2[a]\" -map \"[overlay]\" -map \"[a]\"  -c:v png -c:a aac -strict experimental -shortest " . escapeshellarg(storage_path($video_path));
    ...
    public static function generateThumbnail(Keyword $keyword)
    {
        ...
        //generate unique name for image
        $image_path = "$folder/thumbnail.png";
    ...
    public static function generateTitle($keyword)
    {
        ...
        $image_path = storage_path('app/images/0_title_image.png');
        $image->resize(1280, 720);
        $image->save($image_path);


        $ffmpegCommand = env('FFMPEG_BINARIES') . " -y -framerate 1/5 -i $image_path -r 30 -c:v png -pix_fmt rgba " . escapeshellarg(storage_path('app/videos/0_title_video.mp4'));
